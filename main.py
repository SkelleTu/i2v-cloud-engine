...
def health():
    try:
        ...
        cuda = False
        gpu = None
    return {"ok": True, "cuda": cuda, "gpu": gpu, "model": MODEL, "backend": BACKEND}
...
def _generate_diffusers(image_bytes: bytes, prompt: str, negative_prompt: Optional[str], frames: int, steps: int, guidance: float) -> bytes:
    # Placeholder deliberately isolated: the exact Wan 2.2 pipeline depends on the
    # CUDA runtime, VRAM and installed Diffusers version available in the deployment.
...
async def generate(
    ...
    if use_diffusers:
        video = _generate_diffusers(image_bytes, prompt, negative_prompt, num_frames, num_inference_steps, guidance_scale)
    else:
        video = _generate_hf(image_bytes, prompt, negative_prompt, num_frames, num_inference_steps, guidance_scale)
